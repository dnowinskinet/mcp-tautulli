...
### Added
- `tautulli_stream_data` — detailed per-stream performance metrics (bitrate, codec, resolution, transcode decisions) by `row_id` or `session_key`
- `tautulli_history` now surfaces `transcode_decision` and `row_id`; new `include_performance` flag fetches per-record bitrate, and new `include_ip` flag (off by default) shows client IP
- Shared `httpx.AsyncClient` via FastMCP lifespan with optional `Context` logging — connection reuse across calls
- `tautulli://activity` and `tautulli://server` MCP resources
- Test suite (pytest, ~98% coverage)

### Changed
- Console entry point now runs `main()` so the installed `mcp-tautulli` command validates required env vars before starting

### Credits
- Stream-performance work contributed by [@detailobsessed](https://github.com/detailobsessed) (issue #2)
